...
if __name__ == '__main__':
    ...
    #     save_dir=f"{base}\\FULL_VIT_TEST_MSKD_logs",
    #     name=f"FINAL_VIT_csv_max_acc",
    # )

    # checkpoint_cb = pl.callbacks.ModelCheckpoint(
    #     dirpath=os.path.join(base, f"FINAL_VIT_MSKD_CHKT"),
    ...
    model = transfer_learning_load(
        k_classes=4,
        weights_pth="FINAL_MODELS\\final_VIT_max_accuracy\\FINAL_VIT_CHKT\\weights_masked_head_upd_wo_CLS_max_acc.ckpt",
        hparams_pth="FINAL_MODELS\\final_VIT_max_accuracy\\FULL_VIT_TEST_logs\\FINAL_VIT_csv_masked_head_upd_wo_CLS_max_acc\\version_1\\hparams.yaml"
    )

    base = "POMPAF_final_VIT"
    # CSV logger
    logger_csv = CSVLogger(
        save_dir=f"{base}\\FULL_VIT_TEST_MSKD_TF_logs",
        name=f"FINAL_VIT_csv_max_acc",
    )

    checkpoint_cb = pl.callbacks.ModelCheckpoint(
        dirpath=os.path.join(base, f"FINAL_VIT_MSKD_TF_CHKT"),
        filename=f"weights_max_acc",
        monitor="val_accuracy",
        mode="max",
        save_top_k=1
