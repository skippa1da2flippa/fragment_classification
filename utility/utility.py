...
def generate_connection_discrete(
    ...
    global_nodes: Tensor = torch.cat([patches_emb[:, 0, :], other_global_nodes], dim=1)
    if central_node_mode == "mean":
        central_node: Tensor = global_nodes.mean(dim=1)
    else:
    ...
    cos_identity: Tensor = torch.eye(
        ...
        cosine_similarities.shape[2], 
        device=cosine_similarities.device,
        dtype=cosine_similarities.dtype
    )
    cosine_similarities *= (1 - cos_identity) # TODO so the diagonal is not taken into consideration

    cos_sim_sum: Tensor = cosine_similarities.sum(dim=[1, 2], keepdim=True)
    cos_sim_sum = cos_sim_sum.squeeze(dim=-1)
    avg_cosine_sim: Tensor = cos_sim_sum / ((cosine_similarities.shape[1]**2) - cosine_similarities.shape[1]) # TODO so the diagonal is not taken into consideration

    samples: Tensor = cosine_similarities.flatten(start_dim=1)
    samples_identity: Tensor = cos_identity.flatten(start_dim=1)
    samples_agg: Tensor = (samples - avg_cosine_sim) * (1 - samples_identity)

    var_cosine_sim: Tensor = (
        samples_agg**2
    ) / (samples_agg.shape[-1]**2 - samples_agg.shape[-1])  # TODO so the diagonal is not taken into consideration
    ...
    # TODO add here the concatenation 
